...
//			user.setPageOf(Constants.PAGE_OF_USER_ADMIN);
	...
	public void testUpdateCollectionProtocolAddCollectionProtocolEvent()
	{
	    try 
		{
	    	...
	    	Logger.out.info("Domain object successfully updated ---->"+updatedCollectionProtocol);
	    	assertTrue("Domain object updated successfully", true);
	    	BizTestCaseUtility.setObjectMap(updatedCollectionProtocol,CollectionProtocol.class);//bug 12073 and 12074
	    } 
	    catch (Exception e)
	    {
	    	Logger.out.error(e.getMessage(),e);
	...
	{
		CollectionProtocol collectionProtocol = new CollectionProtocol();
    	CollectionProtocol cachedCollectionProtocol = (CollectionProtocol) BizTestCaseUtility.getObjectMap(CollectionProtocol.class);
