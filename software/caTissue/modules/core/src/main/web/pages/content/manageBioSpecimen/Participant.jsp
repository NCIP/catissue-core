...
<%
	...
	if(pageOf.equals(Constants.PAGE_OF_PARTICIPANT_CP_QUERY))
	{
		...
		{
	formName = Constants.PARTICIPANT_AUTHORIZE_ACTION;
	if(pageOf.equals(Constants.PAGE_OF_PARTICIPANT_CP_QUERY))
	{
			...
			if(request.readyState == 4)
			{
				...
				if(request.status == 200)
				{
					...
					if(responseString=="<%=Constants.PARTICIPANT_CONSENT_ENTER_RESPONSE%>")
					{
						...
						anchorTag.innerHTML=responseString+"<input type='hidden' name='" + verificationKey + "' value='Consent' id='" + verificationKey + "'/>";
						spanTag.appendChild(anchorTag);
					}
					else //No Consent
					{
						spanTag.innerHTML=responseString+"<input type='hidden' name='" + verificationKey + "' value='Consent' id='" + verificationKey + "'/>";
					}
