...
function handleToggle(ele){
	if(ele.checked){
		...
		$("updatedMessage").show();
		$("updatedSection").show();
		$('customizeChkId').checked=true;
		onChkBoxClickForCP('customizeChkId','userIds','siteIds','roleIds','actionIds');
		$('customizeChkId').disabled=true;
		deselectAllOptsOfSelBox($('userIds'));
		$('userIds').disabled=true;
		<%--var roleSelBoxObj = document.getElementById('roleIds');
		...
		$("updatedMessage").hide();
		$("updatedSection").hide();
		$('customizeChkId').disabled=false;
		$('customizeChkId').checked=false;
		<%--$('roleIds').disabled=false;--%>
		onChkBoxClickForCP('customizeChkId','userIds','siteIds','roleIds','actionIds');
	}
	$("stemName").options[0].selected = true;
	$("gridGroups").length=0
	$("gridGroups").options[0] = new Option("--Select--","-1");
	clearListForUserCP($('userIds'));
}
function populateGridGroups(stem){
	$("gridGroups").length=0
	$("gridGroups").options[0] = new Option("--Select--","-1");
	var groups = gridGrouperMap[stem];
	for(i=1; i<=groups.length ; i++){
		$("gridGroups").options[i] = new Option(groups[i], groups[i]);
	}
}

function enableGridDelete(){
	shouldEnable = false;
	$$('.deleteGridPrivilege').each(function(checkbox){
		if(checkbox.checked) shouldEnable = true;
	});
	if(shouldEnable){
		$('deleteGridButton').disabled = false;
	}else{
		$('deleteGridButton').disabled = true;
	}
}
</script>
...
<%
	...
	List<String> stems = (List) request.getAttribute(Constants.GRIDGROUPERSTEMS);
%>
			...
			<table summary="" cellpadding="0" cellspacing="0" border="0"
				...
				<tr>
					...
					<table width="100%" border="0" cellpadding="4" cellspacing="0"
						...
						<!-- DISABLED TEMP TO FIX 21042 BUG. NEED TO FIX LATER   -->
						<tr><td colspan="7" class="black_ar_t"><input id="gridGrouperChkbox" type="checkbox" onclick="handleToggle(this);"/> <bean:message
								key="assignPrivileges.gridPrivilege" /> </td></tr>
						...
						<tr>
							...
							<td width="7%" align="left" class="black_ar_t"><div id="onloadMessage"><bean:message
								key="Site.header" /></div><div style="display:none;" id="updatedMessage"><bean:message
								key="assignPrivileges.stem" /><br><br><bean:message
								key="assignPrivileges.group" /></div></td>
							<td width="28%" align="left" class="black_ar_t"><div id="onloadSection"><select
							...
							<div id="updatedSection" style="display:none">
								<select id="stemName" class="formFieldSizedNew" style="width:10em;" onchange="getGroupsForThisStem(this,'edit');">
									<options>
								...
								<br><br>  <select id="gridGroups" class="formFieldSizedNew" style="width:10em;" onchange="getUsersForThisGroup(this,'edit');">
									<options>
										<option selected value="-1">--Select--</option>
									</options>
								</select>
				...
				<tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="3">
						<tr class="tableheading">
							<td width="6%" class="black_ar_b"><label for="delete"
								align="left"><bean:message key="app.select" /></label></td>
							<td width="29%" class="black_ar_b" align="left"><bean:message
								key="assignPrivileges.stem" /></td>
							<td width="25%" class="black_ar_b" align="left"><bean:message
								key="assignPrivileges.group" /></td>
							<td width="40%" class="black_ar_b" align="left"><bean:message
								key="app.Privileges" /></td>
							<td class="black_ar_b">&nbsp;</td>
						</tr>
						<tr>
							<td colspan="5" width="100%">
							<div
								style="height: 80px; background-color: #ffffff;overflow: auto;">
							<table id="gridSummaryTableId" border="0" width="100%" cellspacing="0" cellpadding="0">
								<tbody>
									<%
												List list1 = (List) request
												.getAttribute(Constants.GRID_PRIVILEGE_DATA_LIST_ONLOAD);
										if (list1 != null) {
											int valueCounter = list1.size();
											for (int i = 0; i < valueCounter; i++) {
												String[] arr = (String[]) (list1.get(i).toString().split(";;"));
									%>
									<%
									if (i % 2 != 0) {
									%>
									...
									<tr>
										...
										<td width="6%" class="black_ar">
											<input type='checkbox' name="<%=arr[0]%>:<%=arr[1]%>" class="deleteGridPrivilege" onclick="enableGridDelete();"/>
										</td>
										<td width="27%" class="black_ar">
										<span><%=arr[0]%></span></td>

										<td width="22%" class="black_ar"><span><%=arr[1]%></span></td>
						...
						<tr>
							<td class="black_ar" colspan="6"><html:button
								property="deleteGridButton" styleId="deleteGridButton"
								styleClass="black_ar" disabled="true" onclick="removeGridPrivilege('${requestScope.operation}');">
								<bean:message key="buttons.delete" />
							</html:button></td>
						</tr>
						<tr>
