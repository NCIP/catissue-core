...
var sppLength = "";
function selectAllSPP()
{
 	var selectedSPP = document.getElementById('specimenProcessingProcedure');
	sppLength=selectedSPP.options.length;
	if (selectedSPP != null)
	{
		for (i = selectedSPP.options.length-1; i >= 0; i--)
		{
			selectedSPP.options[i].selected=true;
		}
	}
 }
...
{
	selectAllSPP();
	var action = "SaveProtocolEvents.do?pageOf=defineEvents&operation=add&sppLength="+sppLength;
	document.forms[0].action = action;
	document.forms[0].submit();
}
