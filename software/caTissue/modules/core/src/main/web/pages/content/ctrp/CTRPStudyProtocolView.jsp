<%@page import="gov.nih.nci.coppa.services.pa.StudyProtocol"%>
...
<head>	
<%
	StudyProtocol[] studyProtocols = (StudyProtocol[]) request.getAttribute("COPPA_STUDY_PROTOCOLS");
	List dataList = null;
	if (studyProtocols != null) {
	dataList = Arrays.asList(studyProtocols);
	}
	String pageName = "CTRPStudyProtocol.do";	
	String[] columns = {"NCI Identifier", "Official Title", "Acronym", "Start Date"  };
	List columnList = Arrays.asList(columns);
	String title = "ctrp.protocol.select";
%>
	...
	function closeWindowPopup(){
		this.parent.dhtmlmodal.close(this.parent.window.document.getElementById('CTRPSelectProtocol'));
			...
			<table border="0" cellpadding="0" cellspacing="0">
		      <tr>
				<td class="td_table_head" nowrap="true">
					<span class="wh_ar_b">
						NCI Enterprise Study Protocol Matches
					</span>
				</td>
		        <td>
...
<html:form action="CTRPStudyProtocol.do" style="margin:0;padding:0;">
      <tr>
      ...
      <tr>
            <td width="10%" align="left" class="black_ar_b" nowrap="nowrap">
            	<bean:message key="ctrp.protocol.popup.title"/>
            </td>
            <td width="20%" align="left">
            	  <html:text styleClass="black_ar" maxlength="255"  size="40" styleId="entityName" property="entityName"/> 
            </td>
	...
	<%
		...
		<tr class="tableheading">
			...
			<%
						...
						{
							StudyProtocol studyProtocol = (StudyProtocol) dataList.get(j);
							remoteIdentifier =	COPPAUtil.getRemoteIdentifier(studyProtocol);
					%>
			<tr>
				<td>
				...
				<td class="black_ar" nowrap="nowrap">	
					<%= COPPAUtil.getNCIIdentitifer(studyProtocol)%>
				</td>
				<td class="black_ar" nowrap="nowrap">	
					<%= COPPAUtil.getTitle(studyProtocol)%>
				</td>
				<td class="black_ar" nowrap="nowrap">	
		...
		<td width="90%">
			...
			<tr>
					<td width="5%" nowrap align="left" valign="top">
						<html:button property="linkButton" styleId="linkButton" value="Link to Existing Protocol" styleClass="blue_ar_b" onclick="submitOperation('Link')" onkeypress="submitOperation('Link')" disabled="true"/>
						<c:choose>
							 <c:when test="${(operation ne 'edit')}">
								<html:button property="createButton" styleId="createButton" value="Create New Protocol" styleClass="blue_ar_b" onclick="submitOperation('Create')" onkeypress="submitOperation('Create')" disabled="true"/>
							</c:when>
						</c:choose>
