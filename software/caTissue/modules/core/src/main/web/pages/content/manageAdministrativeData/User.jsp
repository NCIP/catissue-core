...
<script language="JavaScript">
	...
	var ctrpSearchNoMatchMessage = '<bean:message key="ctrp.search.nomatch.message"/>';
...
<script>
function openLinkCOPPAWindow()
{
	  var inputParams = "&emailAddress="+document.getElementById('emailAddress').value+"&lastName="+document.getElementById('lastName').value+"&firstName="+document.getElementById('firstName').value;
	  var findUserURL = 'CTRPUser.do?operation=${operation}&pageOf=${pageOf}'+inputParams;
	  openLinkCOPPAWindow=dhtmlmodal.open('CTRPSelectUser', 'iframe', findUserURL,'Select NCI Enterprise User', 'width=600px,height=300px,max-height=200px,center=1,resize=0,scrolling=1')
	  openLinkCOPPAWindow.controls.onclick=function(){dhtmlmodal.close(this._parent)}
   	  openLinkCOPPAWindow.onclose=function()
      {
            return true;
      }
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<html:form action='${requestScope.formName}'>
		...
		<html:hidden property="targetIdp" />
		...
		<tr>
			...
			<table width="100%" border="0" cellpadding="3" cellspacing="0"
				...
				<tr>
					...
					<table width="100%" border="0" cellpadding="3" cellspacing="0">
					<logic:notEmpty name="userForm" property="targetLoginName">
						<tr>
							<td align="center" class="black_ar"></td>
							<td align="left" class="black_ar"><bean:message key="user.identity.provider" /></td>
							<td width="22%" align="left" class="black_ar">
								<label>
								 	${requestScope.userForm.targetIdp}
								</label>
							</td>
							<td align="left" class="black_ar">&nbsp;</td>
							<td align="center" class="black_ar"></td>
							<td align="left" class="black_ar"><bean:message key="user.login.name" /></td>
							<td width="22%" align="left" class="black_ar">
						...
						<tr>
							...
							<td align="left">
								<c:if test="${(isCOPPAEnabled == true)}">
									<div id="findUserDiv">
										<html:link href="#" styleClass="viewprom" styleId="lookupNCI" titleKey="ctrp.check.coppa.message"
											onclick="openLinkUserWindow('${operation}','${pageOf}');">
											<bean:message key="ctrp.check.coppa.text" />
										</html:link>
									</div>
						...
						<tr>
							...
							<c:choose>
								<c:when test="${(isCOPPAEnabled == true)}">
									<td>
											<input property="displayinstitutionId" type="text" id="displayinstitutionId" name="displayinstitutionId" size="27" onmouseover="showTip(this.id)"/>
											<input property="institutionId" type="hidden" id="institutionId" name="institutionId"/>
											<input property="remoteInstitutionId" type="hidden" id="remoteInstitutionId" name="remoteInstitutionId"/>
									</td>
								</c:when>
								<c:otherwise>
							...
							<table width="100%" border="0" cellpadding="3" cellspacing="0">
								...
								<tr>
									<td colspan="2" class="buttonbg">
										...
										<!-- 
										 <html:button property="submitButton" styleId="submitButton" value="Submit" styleClass="blue_ar_b" onclick="submitUserOperation()" onkeypress="submitUserOperation()"/>
										 -->
<!-- &nbsp;|<logic:notEqual name="pageOf"
										value='${requestScope.pageOfSignUp}'>
										<logic:notEqual parameter="openInCPFrame" value='yes'>
...
window.onload=function (){
		...
		if (isCOPPAEnabled){
			Ext.onReady(populateInstitutionExtCombobox);
		}
	}
