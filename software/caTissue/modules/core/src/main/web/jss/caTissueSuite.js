...
// end here
 ...
 function getCustomRole(roleSelBoxId,actionSelBoxObj)
 {
   ...
   // Function to send request to get Privileges Summary.
      ...
      {
    	  if($("gridGrouperChkbox") != null) {
	    	  if($("gridGrouperChkbox").checked){
	    		  getGridPrivilegeSummary(operation);
	    		  return;
	    	  }
    	  }
   ...
   // Function to send request to get Privileges Summary.
      ...
      function getGridPrivilegeSummary(operation)
      {
            var cpOperation="addGridPrivilege";
            var form=document.forms[0];
      		var pageOf = form.pageOf.value;
            ...
            stemName=$("stemName").value;
            groupName=$("gridGroups").value;
            actionListCtrl=document.getElementById("actionIds");
            userListCtrl=document.getElementById("userIds");
            var selectedRoleIds=$("roleIds").value;
            var selectedActionIds = new Array();
            var selectedUserIds = new Array();
            var url="ShowAssignPrivilegePage.do?pageOf="+pageOf;
            var data="";
			...
			data="cpOperation="+cpOperation+"&stemName="+stemName+"&groupName="+groupName+"&selectedActionIds="+selectedActionIds+"&operation="+operation+"&selectedUserIds="+selectedUserIds+"&selectedRoleIds="+selectedRoleIds;

			isErrors = validateMethodForPriv(pageOf,new Array(),getSelElementsList($("roleIds")),selectedActionIds,new Array(),null,isAllCPChecked,true);
		          ...
		          request.onreadystatechange = function requestHandler()
		          {
		              ...
		              {       //When response is loaded
		                 if(request.status == 200)
		                 {
		                     ...
		                     updateGridSummaryTable(response);
		                 }
		              }
		          }
		          request.open("POST",url,true);
   ...
   // Function to send request to delete Privileges.

      function removeGridPrivilege(operation)
      {
            var cpOperation="removeGridPrivileges";
            var form=document.forms[0];
      		var pageOf = form.pageOf.value;
      		var deletedItems=new Array();
      		count=0;
      		$$('.deleteGridPrivilege').each(function(checkbox){
      			if(checkbox.checked) deletedItems[count++]=checkbox.name
      		});
			...
			data="cpOperation="+cpOperation+"&deletedGridPrivileges="+deletedItems+"&operation="+operation;
			request=newXMLHTTPReq();
	          request.onreadystatechange = function requestHandler()
	          {
	              if(request.readyState == 4)
      ...
      function updateGridSummaryTable(response){
    	  for(i=$("gridSummaryTableId").rows.length;i>0;i--) {
         	 $("gridSummaryTableId").deleteRow(i-1);
          }
    	  if(response==null || response == "") return;
          rowArray = response.split(";;;");
          for(i=0 ; i<rowArray.length ; i++){
         	 var x=$("gridSummaryTableId").insertRow(i);
         	 if(i%2!=0) x.setAttribute("class","tabletd1");
         	 cellArray = rowArray[i].split(';;');
         	 var c1=x.insertCell(0);
         	 var c2=x.insertCell(1);
         	 var c3=x.insertCell(2);
         	 var c4=x.insertCell(3);
         	 var c5=x.insertCell(4);
         	 c1.setAttribute("width","6%");
         	 c1.setAttribute("class","black_ar")
         	 c1.innerHTML="<input type='checkbox' class='deleteGridPrivilege' name='"+cellArray[0]+":"+cellArray[1]+"' onclick='enableGridDelete();'/>";
         	 c2.setAttribute("width","27%");
         	 c2.setAttribute("class","black_ar")
         	 c2.innerHTML="<span>"+cellArray[0]+"</span>";
         	 c3.setAttribute("width","22%");
         	 c3.setAttribute("class","black_ar")
         	 c3.innerHTML="<span>"+cellArray[1]+"</span>";
         	 c4.setAttribute("width","40%");
         	 c4.setAttribute("class","black_ar")
         	 c4.innerHTML="<span>"+cellArray[2]+"</span>";
          }
      }
    
      function getGroupsForThisStem(stemSelectBox, operation){
    	  if($("stemName").value == -1) return;
    	  $("gridGroups").length=0
    	  $("gridGroups").options[0] = new Option("Loading..","-1");
    	  clearListForUserCP($("userIds"));
    	  var cpOperation="getGroupsForStem";
          var form=document.forms[0];
          var pageOf = form.pageOf.value;
          var stemName=document.getElementById("stemName").value;
          var url="ShowAssignPrivilegePage.do?pageOf="+pageOf;
          
          data="cpOperation="+cpOperation+"&stemName="+stemName+"&operation="+operation;
          ...
          request.onreadystatechange = function requestHandler()
          {
              ...
              {       //When response is loaded
                 if(request.status == 200)
                 {
                     ...
                     $("gridGroups").length=0
                     if(response == ""){
                    	 $("gridGroups").options[0] = new Option("No Groups Found","-1");
                     }else{
                    	 $("gridGroups").options[0] = new Option("--Select--","-1");
                    	 $("gridGroups").options[0].selected=true;
                    	 var groups = response.split(";;;");
                         for(i=0; i<groups.length ; i++){
                        	 groupsArray=groups[i].split(":")
                     		$("gridGroups").options[i+1] = new Option(groupsArray[groupsArray.length-1], groups[i]);
                         }
                     }
                 }
              }
      ...
      function getUsersForThisGroup(groupSelectBox, operation){
    	  if($("gridGroups").value == -1) return;
    	  var eleOfUserSelBox = document.getElementById('userIds')
    	  clearListForUserCP(eleOfUserSelBox);
    	  $("userIds").length=0
    	  $("userIds").options[0] = new Option("Loading..","-1");
    	  var cpOperation="getUsersForGroup";
          var form=document.forms[0];
          var pageOf = form.pageOf.value;
          var groupName=document.getElementById("gridGroups").value;
          var url="ShowAssignPrivilegePage.do?pageOf="+pageOf;
          
          data="cpOperation="+cpOperation+"&groupName="+groupName+"&operation="+operation;
          ...
          request.onreadystatechange = function requestHandler()
          {
              ...
              {       //When response is loaded
                 if(request.status == 200)
                 {
                     ...
                     $("userIds").length=0
                     if(response == ""){
                    	 $("userIds").options[0] = new Option("No Users Found","-1");
                     }else{
                    	 var users = response.split(";;;");
                         for(i=0; i<users.length ; i++){
                        	 var usersArray=users[i].split("CN=")
                     		$("userIds").options[i] = new Option(usersArray[usersArray.length-1], users[i]);
                         }
                     }
                 }
              }
	...
	function validateMethodForPriv(pageOf,selectedSiteIds,selectedRoleIds,selectedActionIds,selectedUserIds,selectedCPIds,isAllCPChecked,isCustChecked)
    {
        ...
        var isGridGrouperChecked = false;
        
	  	  if($("gridGrouperChkbox") != null) {
	  		isGridGrouperChecked = document.getElementById("gridGrouperChkbox").checked;
		  }
    	...
    	if(pageOf!=null && pageOf=="pageOfAssignPrivilegePage")
		{
			...
			if(selectedSiteIds.length=='0' && !isGridGrouperChecked)
             {
             	 errorFlagForSite=true;
             }
			if(isCustChecked)
			{
				if(selectedUserIds.length=='0' && !isGridGrouperChecked)
				 {
