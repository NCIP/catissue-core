containerKeys=function (obj)
{
	var keys = [];
	var tempObj = new Hashtable();
	var customKeys=obj.keys();
	var existingKeys=tempObj.keys();
	for (var i=0;i <customKeys.length;i++)
		{
			if(existingKeys.indexOf(customKeys[i])== -1)
			{
...
if(dataTable != null)
{
	...
	{
		var keys = containerKeys(dataTable);
        var  j=0;
...
function onCustomListBoxChangeForContainer(element)
{
//Get the element identifier
var elementId = element.id;
