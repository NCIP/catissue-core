...
public class SubmitUserAction extends XSSSupportedAction
{
			...
			HttpServletRequest request, HttpServletResponse response) throws AssignDataException
	{
		try
		{
			...
			//Create remote institution if not present in caTissue and set instituion id in form
			UserForm userForm = (UserForm)form;
			long institutionId = userForm.getInstitutionId();
			long remoteInstitutionId = userForm.getRemoteInstitutionId();
			System.out.println("Form Params:insId:"+institutionId+":remoteInsId:"+remoteInstitutionId);
			if ((institutionId == 0) && (remoteInstitutionId != 0)) {
				// User selected a remote institution not present in caTissue
				System.out.println("Creating Remote Institution with bizlogic");
				final InstitutionBizLogic bizlogic = new InstitutionBizLogic();
				try {
					Institution inst = new COPPAServiceClient()
							.getRemoteInstituionById("" + remoteInstitutionId);
					System.out.println("Fetched Remote Ins details:"+inst.getName());
					bizlogic.insert(inst);
					userForm.setInstitutionId(inst.getId());
					System.out.println("Institution Created:" + inst.getId());
				} catch (Exception e) {
					throw new ApplicationException(
							ErrorKey.getErrorKey("errors.item"), e,
							"Failed to get remote NCI institution in base Add/Edit.");
				}
			}
			...
			throws BizLogicException, AssignDataException
	{
		...
		IUIObjectFactory uiFactory= AbstractFactoryConfig.getInstance().getUIObjectFactory();
		UIObject uiObject= uiFactory.getUIObject(form);
		bizLogic.insert(userDTO,uiObject, sessionDataBean);
	...
	private void updateUser(UserDTO userCurrent, User userOld, HttpSession session,
			AbstractActionForm form) throws BizLogicException, AssignDataException
	{
		final IFactory factory = AbstractFactoryConfig.getInstance().getBizLogicFactory();
		final IBizLogic bizLogic = factory.getBizLogic(form.getFormId());
		final SessionDataBean sessionDataBean = (SessionDataBean) session
		...
		IUIObjectFactory uiFactory= AbstractFactoryConfig.getInstance().getUIObjectFactory();
		UIObject uiObject= uiFactory.getUIObject(form);
		bizLogic.update(userCurrent, userOld,uiObject, sessionDataBean);
	}
