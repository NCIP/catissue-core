				...
				for (Long containerId : entityIdsVsContId.values())
				{
					if (containerId != null)
					{
						...
						if (formContextList == null || formContextList.isEmpty())
						{
							...
							StudyFormContext studyFormContext = studyFormInstFact.createObject();//new StudyFormContext();
						...
						else
						{
							...
							Collection<CollectionProtocol> cpColl = studyFormContext.getCollectionProtocolCollection();
							if(cpColl==null)
							{
								cpColl= new HashSet<CollectionProtocol>();
							}
							cpColl.clear();
