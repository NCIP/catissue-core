...
public class AliquotBizLogic extends CatissueDefaultBizLogic
{
			...
			NewSpecimenBizLogic newSpecimenBizLogic, DAO dao) throws BizLogicException
	{
		try
		{
			...
			SpecimenCollectionGroup specimenCollGroup =instFact.createObject();// new SpecimenCollectionGroup();
			...
			if (!edu.wustl.catissuecore.util.global.Variables.isSpecimenLabelGeneratorAvl)
			{
				...
				//This IF-CONDITION was commented before and above statement commented by me
				totalAliquotCount = totalAliquotCount  + 1;
			...
			int aliqoutCounter, List<SpecimenPosition> specimenPosList)
	{
		...
		else
		{
			if(aliquot.getAliquotsInSameContainer())
			{
				...
				SpecimenPosition newSpecimenPosition = instFact.createObject();//new SpecimenPosition();
