...
public abstract class BaseShipmentBizLogic extends CatissueDefaultBizLogic
{
	...
	{
		ShipmentRequestUIObject reqUIObject=new ShipmentRequestUIObject();
		insert(obj,dao,reqUIObject,sessionDataBean);
	}
	/**
	...
	protected void insert(Object obj, DAO dao,Object uiObject, SessionDataBean sessionDataBean)
			throws BizLogicException
	{
		try
		{
			ShipmentRequestUIObject reqUIObject=(ShipmentRequestUIObject)uiObject;
			...
			if (obj != null && obj instanceof Shipment)
			{
				...
				if (( (Shipment) obj ).getShipmentRequest() != null)
				{
					...
					reqUIObject.setRequestProcessed( Boolean.TRUE );
					...
					if (specimen != null)
					{
							...
							!( position.getStorageContainer().getStorageType().getName().trim().equals( Constants.SHIPMENT_CONTAINER_TYPE_NAME ) ) ))
						{
							...
							InstanceFactory<SpecimenPosition> instFact = DomainInstanceFactory.getInstanceFactory(SpecimenPosition.class);
									final SpecimenPosition tempPosition = instFact.createObject();//new SpecimenPosition();
						...
						if (operation.equals( edu.wustl.catissuecore.util.global.Constants.ADD ))
						{
							...
							{
								InstanceFactory<SpecimenPosition> instFact = DomainInstanceFactory.getInstanceFactory(SpecimenPosition.class);
								position = instFact.createObject();//new SpecimenPosition();
