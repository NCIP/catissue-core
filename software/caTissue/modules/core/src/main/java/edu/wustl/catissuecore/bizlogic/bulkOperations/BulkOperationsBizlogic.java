...
public class BulkOperationsBizlogic extends SpecimenEventParametersBizLogic
{
			...
			Map<String, String> eventSpecificData) throws ParseException, BizLogicException
	{
		...
		{
			transferEventParameters = (TransferEventParameters)DomainInstanceFactory.getInstanceFactory(TransferEventParameters.class).createObject();//new TransferEventParameters();
			...
			fromContainer = scInstFact.createObject();//new StorageContainer();
			...
			Map<String, String> eventSpecificData) throws ParseException
	{
		...
		for (int i = 0; i < specimenIds.size(); i++)
		{
			disposalEventParameters = (DisposalEventParameters)DomainInstanceFactory.getInstanceFactory(DisposalEventParameters.class).createObject();//new DisposalEventParameters();
			specimenId = (String) specimenIds.get(i);
			this.setCommonEventParameters(specimenId, userId, dateOfEvent, timeInHours,
