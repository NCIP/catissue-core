...
public class SpecimenArrayTypeBizLogic extends CatissueDefaultBizLogic
{
			...
			throws BizLogicException
	{
		...
		{
			this.logger.error(daoExp, daoExp);
			...
			throws BizLogicException
	{
		try
		{
			...
			dao.update(arrayType.getCapacity(),((SpecimenArrayType)oldObj).getCapacity());
			dao.update(arrayType,oldObj);
		}
		catch (final DAOException daoExp)
		{
	...
	protected boolean validate(Object obj, DAO dao, String operation) throws BizLogicException
	{
		...
		final Collection<String> specimenTypeCollection = specimenArrayType.getSpecimenTypeCollection();

		if ((specimenTypeCollection != null) && (specimenTypeCollection.size() > 0))
		{
			final Iterator<String> arrayTypeIterator = specimenTypeCollection.iterator();
			while (arrayTypeIterator.hasNext())
			{
				final String temp = arrayTypeIterator.next();
				if (temp != null)
				{
