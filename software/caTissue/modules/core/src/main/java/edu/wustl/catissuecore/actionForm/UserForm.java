...
public class UserForm extends AbstractActionForm
{
     ...
     * Institution name of the user.
     */
    private long remoteInstitutionId;
...
/**
    ...
    public long getRemoteInstitutionId() {
		return remoteInstitutionId;
	}

	public void setRemoteInstitutionId(long remoteInstitutionId) {
		this.remoteInstitutionId = remoteInstitutionId;
	}
    ...
    public void setAllValues(final AbstractDomainObject abstractDomain)
    {
        if (!Constants.PAGE_OF_CHANGE_PASSWORD.equals(getPageOf()))
        {
            ...
            {
                role = UserUtility.getRoleId(user);
            }
            setRemoteData(user);
        }
    ...
    private void setUserData(final User user)
    {
                ...
                || ((getPageOf() != null) && (Constants.PAGE_OF_USER_ADMIN.equals(getPageOf()))))
        {
            ...
            setCmts(user);
            setUserRole(UserUtility.getRoleId(user));
            if (getFormId() == Constants.APPROVE_USER_FORM_ID)
            {
                ...
                else
                {
                    ...
                    if (Constants.PAGE_OF_SIGNUP.equals(getPageOf()))
                    {
                        if ("yes".equalsIgnoreCase(idpSelection))
                        {
                            ...
                            //validateForUserExistanceInGridGrouper(errors);
                        }
                    }
    ...
    /*
    private void validateForUserExistanceInGridGrouper(final ActionErrors errors) throws Exception {
    	if (getGrouperUser().equals("yes")) {
    		boolean flag = GridGrouperUtil.isIdentityPresentInGridGrouper(this.targetLoginName);
    		if (!flag) {
    			 errors.add(ActionErrors.GLOBAL_ERROR,
                         new ActionError("app.target.loginname.notingrouper", ""));
    		}
    	}
    }*/
...
//    }
    ...
    private void setRemoteData(final User user)
    {
        if (!CommonUtilities.isNull(user.getRemoteManagedFlag()))     {
        	remoteManagedFlag = user.getRemoteManagedFlag().booleanValue();
        }
        if (!CommonUtilities.isNull(user.getDirtyEditFlag()))     {
        	dirtyEditFlag = user.getDirtyEditFlag().booleanValue();
        }
        if (!CommonUtilities.isNull(user.getRemoteId()))     {
        	remoteId = user.getRemoteId().longValue();
        }
    }
