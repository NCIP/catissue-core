	...
	*/
	private void updateReport() throws Exception
	{
		...
		try
		{
			...
			{
				InstanceFactory<BinaryContent> instFact = DomainInstanceFactory.getInstanceFactory(BinaryContent.class);
				final BinaryContent binaryContent = instFact.createObject();//new BinaryContent();
			...
			if ((CaTIESProperties.getValue(CaTIESConstants.CATIES_SAVE_XML_CONTENT))
					.equalsIgnoreCase("true"))
			{
				InstanceFactory<XMLContent> instFact = DomainInstanceFactory.getInstanceFactory(XMLContent.class);
				final XMLContent xmlContent = instFact.createObject();//new XMLContent();
			...
			throws Exception
	{
		...
		try
		{
					...
					.hasNext();)
			{
				...
				//"ConceptReferent") ;
				InstanceFactory<ConceptReferent> instFact = DomainInstanceFactory.getInstanceFactory(ConceptReferent.class);
				final ConceptReferent conceptReferentToAdd = instFact.createObject();//new ConceptReferent();
				...
				InstanceFactory<Concept> conceptInstFact = DomainInstanceFactory.getInstanceFactory(Concept.class);
				Concept conceptToAdd = conceptInstFact.createObject();// Concept();
				...
				conceptToAdd.setName(conceptElement
						.getAttributeValue(CaTIESConstants.TAG_ATTRIBUTE_NAME));
							//"name")) ;
				InstanceFactory<SemanticType> semanticTypeInstFact = DomainInstanceFactory.getInstanceFactory(SemanticType.class);
				final SemanticType semanticType = semanticTypeInstFact.createObject();//new SemanticType();
				...
				//"ConceptClassification");
				InstanceFactory<ConceptReferentClassification> crClassInstFact = DomainInstanceFactory.getInstanceFactory(ConceptReferentClassification.class);
				ConceptReferentClassification conceptClassificationToAdd =crClassInstFact.createObject();
