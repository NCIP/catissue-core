...
public class ReportLoaderQueueProcessor extends Thread
{
	...
	public void run()
	{
        ...
        logger.info(">>> 1. Reading files from: " + configFileName);
		configFileName += CaTIESProperties.getValue(CaTIESConstants.SECTION_HEADER_FILENAME);
        logger.info(">>> 2. Reading files from: " + configFileName);
		while (true)
		{
			try
			{
				...
				final List<String> reportLoaderQueueList = this.getReportLoaderQueueIDList();
					...
					for (int i = 0; i < reportLoaderQueueList.size(); i++)
					{
						...
						try
						{
							this.logger.debug("Processing report" + " from Queue with serial no="
									+ reportLoaderQueue.getId());
							participantSet =  (List) reportLoaderQueue.getParticipantCollection();
	 ...
	 */
	private List<String> getReportLoaderQueueIDList() throws Exception
	{
		...
		final List<String> queue = (List<String>) CaCoreAPIService.executeQuery(hqlQuery, String.class);
		return queue;
	}
}
