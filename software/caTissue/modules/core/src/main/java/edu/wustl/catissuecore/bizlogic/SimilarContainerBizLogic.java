...
public class SimilarContainerBizLogic extends StorageContainerBizLogic
{
	...
	throws BizLogicException
	{
		...
		final Map simMap = stUIObject.getSimilarContainerMap();
	...
	throws BizLogicException
	{
		...
		postInsert(obj,dao,sessionDataBean,stUIObject);
	}
	 ...
	 */
	@Override
	synchronized public void postInsert(Object obj, DAO dao, SessionDataBean sessionDataBean,Object uiObject)
	throws BizLogicException
	{
		StorageContainerUIObject stUIObject=(StorageContainerUIObject)uiObject;
		final StorageContainer container = (StorageContainer) obj;
		try
		{
			...
			for (int i = 1; i <= noOfContainers; i++)
			{
				...
				if (checkButton == 2)
				{
					...
					final StorageContainer parentContainer = scInstFact.createObject();//new StorageContainer();
	...
	protected boolean validate(Object obj, DAO dao, String operation,Object uiObject) throws BizLogicException
	{
		try
		{
			StorageContainerUIObject stUIObject=(StorageContainerUIObject)uiObject;
			final StorageContainer container = (StorageContainer) obj;
			final Map similarContainerMap = stUIObject.getSimilarContainerMap();
			...
			for (int i = 1; i <= stUIObject.getNoOfContainers(); i++)
			{
			...
			if (stUIObject.getNoOfContainers() > 1 && similarContainerMap.size() > 0)
			{
				for (int i = 1; i <= stUIObject.getNoOfContainers(); i++)
				{
	...
	{
		StorageContainerUIObject stUIObject=new StorageContainerUIObject();
		return getObjectId(dao,domainObject,stUIObject);
	}
