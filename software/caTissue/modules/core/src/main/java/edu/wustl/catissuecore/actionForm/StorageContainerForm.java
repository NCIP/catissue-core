...
public class StorageContainerForm extends AbstractActionForm implements IPrinterTypeLocation, ISpecimenType
{
	...
	public void setAllValues(AbstractDomainObject abstractDomain)
	{
		...
		this.defaultTemperature = CommonUtilities.toString(container.getTemperatureInCentigrade());
		...
		final Collection<CollectionProtocol> collectionProtocolCollection = container.getCollectionProtocolCollection();
		...
		if (collectionProtocolCollection != null && collectionProtocolCollection.size() > 0)
		{
			...
			final Iterator<CollectionProtocol> it = collectionProtocolCollection.iterator();
			while (it.hasNext())
			{
		...
		final Collection<StorageType> storageTypeCollection = container.getHoldsStorageTypeCollection();
		...
		if (storageTypeCollection != null)
		{
			...
			final Iterator<StorageType> it = storageTypeCollection.iterator();
			while (it.hasNext())
			{
		...
		final Collection<SpecimenArrayType> specimenArrayTypeCollection = container
		...
		if (specimenArrayTypeCollection != null)
		{
			...
			final Iterator<SpecimenArrayType> it = specimenArrayTypeCollection.iterator();
			while (it.hasNext())
			{
