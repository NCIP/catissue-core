...
/**
 * @author sandeep_ranade AppUtility class containing all general purpose
 *         methods.
 */
public final class ReportLoaderUtil {
	...
	.getCommonLogger(ReportLoaderUtil.class);
	 ...
	 *            default Look UP ResultList.
	 * @return number of matching participants
	 * @throws Exception
	 *             throws exception
	 */
	public static Set<Participant> getParticipantList(
			List defaultLookUPResultList) throws Exception {
		Set<Participant> result = null;
		// check for matching participant list
		if (defaultLookUPResultList != null
				&& defaultLookUPResultList.size() > 0) {
			result = new HashSet<Participant>();
			// prepare list of participant object out of DefaultLookupResult
			// List
	 ...
	 *            directory name
	 * @throws IOException
	 *             IOException occured while creating directories
	 */
	 ...
	 *
	 * @param ssn
	 *            Social Security Number to check
	...
	public static String getValidSSN(String ssn) {
		...
		try {
			re = Pattern.compile("[0-9]{3}-[0-9]{2}-[0-9]{4}",
			...
			if (!result) {
				...
				if (result) {
					...
					buff.append(ssn.substring(0, 3)).append("-").append(
							ssn.substring(3, 5)).append("-").append(
									ssn.substring(5, 9));
				}
			} else {
	 ...
	 * @return line
	 */
	public static String getLineFromReport(String reportText,
			String lineToExtract) {
	...
	/**
	 * This method returns the matching SCG (based on SPN and site) present in
	 * the database.
	 *
	 * @param site
	 *            site
	...
	public static SpecimenCollectionGroup getExactMatchingSCG(Site site,
		...
		List<SpecimenCollectionGroup> resultList=null;
		final String scgHql = "select scg"
			+ " from edu.wustl.catissuecore.domain.SpecimenCollectionGroup as scg "
			+ " where scg.specimenCollectionSite.name='" + site.getName()
			+ "' " + " and scg.surgicalPathologyNumber='"
			+ surgicalPathologyNumber + "'";

		resultList = CaCoreAPIService
		.executeQuery(scgHql, SpecimenCollectionGroup.class);

		logger.info("-------------" + scgHql + "   " + resultList.size());
		SpecimenCollectionGroup scg=null;
		if (resultList != null && resultList.size() == 1) {
			scg= resultList.get(0);
		}
		return scg;
	}
	...
	public static Site getSite(String siteName) throws Exception {
		...
		logger.info("-------------" + scgHql);
		final List<Site> resultList = CaCoreAPIService.executeQuery(scgHql,
				Site.class);
	 ...
	 *             Exception
	 */
	public static boolean isPartialMatchingSCG(Participant participant,
		...
		final List<SpecimenCollectionGroup> resultList = CaCoreAPIService
		.executeQuery(scgHql, SpecimenCollectionGroup.class);
	...
	/**
	 * This method returns the respective participant to which SCG is associated
	 * with.
	 *
	 * @param scgId
	 *            scg Id
	...
	public static Participant getParticipant(Long scgId) throws Exception {
		...
		resultList = CaCoreAPIService
		.executeQuery(hqlString, Participant.class);

		Participant participant = null;
		if (resultList != null && resultList.size() > 0) {
			participant = resultList.get(0);
		}
		return participant;
	}
}
