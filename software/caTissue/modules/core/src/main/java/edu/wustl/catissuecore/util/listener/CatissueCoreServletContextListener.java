...
import edu.wustl.catissuecore.util.DatabaseSchemaUpdater;
...
public class CatissueCoreServletContextListener implements ServletContextListener
{
	...
	public void initCatissueParams() throws ClassNotFoundException, DAOException, ParseException
	{
		...
		System
				.setProperty("app.propertiesDir", CommonServiceLocator.getInstance()
						.getPropDirPath());
		System.setProperty("acegi.security.strategy", "MODE_GLOBAL");
	}
	...
	{
		if ("true".equalsIgnoreCase(System.getenv("CATISSUE_SKIP_ENTITY_CACHE"))) {
			return;
		}
		try
		{
	...
	private void createAliasAndPageOfMap()
	{
		...
		Variables.aliasAndPageOfMap.put(Constants.ALIAS_SPP, Constants.PAGE_OF_SPP);
