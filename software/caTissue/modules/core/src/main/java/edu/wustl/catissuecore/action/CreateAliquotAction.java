...
public class CreateAliquotAction extends BaseAction
{
			...
			HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		IUIObjectFactory uiObjectFactory = AbstractFactoryConfig.getInstance().getUIObjectFactory();
		SpecimenUIObject uiObject = (SpecimenUIObject) uiObjectFactory.getUIObject(aliquotForm);

		// Insert Specimen Map
		insertAliquotSpecimen = this.insertAliquotSpecimen(request, sessionDataBean,
				specimenColl,uiObject);
			...
			throws Exception
	{
		...
		if (specimen != null)
		{
			...
			aliquotForm.setScgName(specimen.getSpecimenCollectionGroup().getName());
		}
	...
	private boolean insertAliquotSpecimen(HttpServletRequest request,
			SessionDataBean sessionDataBean, Collection<AbstractDomainObject> specimenCollection,SpecimenUIObject uiObject)
			throws UserNotAuthorizedException
	{
		try
		{
			new NewSpecimenBizLogic().insert(specimenCollection, sessionDataBean, 0, false,uiObject);
			this.disposeParentSpecimen(sessionDataBean, specimenCollection,
					Constants.SPECIMEN_DISPOSAL_REASON, uiObject);
		}
	...
	private void disposeParentSpecimen(SessionDataBean sessionDataBean,
			Collection<AbstractDomainObject> specimenCollection, String specimenDisposeReason,SpecimenUIObject uiObject)
			throws DAOException, UserNotAuthorizedException, BizLogicException
	{
		...
		if (specimen != null && uiObject.getDisposeParentSpecimen())
		{
			new NewSpecimenBizLogic().disposeSpecimen(sessionDataBean,
					(Specimen)specimen.getParentSpecimen(), specimenDisposeReason);
		}
	}
	...
	private Specimen createParentSpecimen(AliquotForm aliquotForm) throws AssignDataException
	{
		...
		String barcode = aliquotForm.getBarcode();
...
//			new Validator();
			...
			final StorageContainer sContainer = scInstFact.createObject();//new StorageContainer();
			// bug 11479
					...
					&& posDim2 != null)
			{
				...
				final SpecimenPosition specPos = instFact.createObject();//new SpecimenPosition();
