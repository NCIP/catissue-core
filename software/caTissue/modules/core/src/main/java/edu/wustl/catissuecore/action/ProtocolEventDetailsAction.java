...
public class ProtocolEventDetailsAction extends BaseAction
{
			...
			HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		else if(!"newEvent".equals(request.getParameter(Constants.PAGE_OF)))
		{
			setSPPInRequest(protocolEventDetailsForm, request);
		}
		else
		{
			protocolEventDetailsForm.setSpecimenProcessingProcedure(null);
	...
	private void setSPPInRequest( ProtocolEventDetailsForm protocolEventDetailsForm,
			HttpServletRequest request) 
	{
		String arr[] = protocolEventDetailsForm.getSpecimenProcessingProcedure();
		if(arr != null && !"newEvent".equals(request.getParameter(Constants.PAGE_OF)))
		{
			Collection<NameValueBean> sppBean = new ArrayList<NameValueBean>();
			Locale locale = CommonServiceLocator.getInstance().getDefaultLocale();
			for (String string : arr) 
			{
					NameValueBean nvb = new NameValueBean(string,string) ;
						nvb.getName().toLowerCase(locale);
						sppBean.add(nvb);
			}
			request.setAttribute(Constants.SELECTED_SPP, sppBean);
		}
	}
			...
			ProtocolEventDetailsForm protocolEventDetailsForm, HttpServletRequest request)
	{
		...
		protocolEventDetailsForm.setSpecimenProcessingProcedure(collectionProtocolEventBean.getSpecimenProcessingProcedure());
