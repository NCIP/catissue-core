			...
			ServletException
	{
		...
		try
		{
			...
			if (object != null)
			{
				...
				final Collection<SpecimenEventParameters> specimenEventCollection = specimen.getSpecimenEventCollection();

				if (specimenEventCollection != null)
				{
					final List<List<String>> gridData = new ArrayList<List<String>>();
					final Iterator<SpecimenEventParameters> it = specimenEventCollection.iterator();
					// int i=1;

					while (it.hasNext())
					{
						final List<String> rowData = new ArrayList<String>();
						final SpecimenEventParameters eventParameters = it
								.next();
