...
public class ParticipantAction extends SecureAction
{
	...
	protected ActionForward executeSecureAction(ActionMapping mapping, ActionForm form,
		...
		if (participantForm.getOperation().equals(Constants.EDIT))
		{
			...
			setConsentForParticipant(bizlogic, String.valueOf(participantForm.getId()), participantForm.getCollectionProtocolRegistrationValues(),
					participantForm.getCollectionProtocolRegistrationValueCounter());
			// Setting Consent Response Bean to Session
			// Abhishek Mehta
//			final Map consentResponseHT = (Map) session.getAttribute(Constants.CONSENT_RESPONSE);//participantForm.getConsentResponseHashTable();
/*			final String collProtId = request.getParameter("cpId");
			if(cpid!=null && !("-1").equalsIgnoreCase(cpid)){
				final Collection consentList = this.getConsentList(bizlogic,
						collProtId);
				if(consentList!=null)
				{
					participantForm.setIsConsentAvailable(Constants.PARTICIPANT_CONSENT_ENTER_RESPONSE);
				}
			}*/
			if (consentResponseHT != null)
			{
				session.setAttribute(Constants.CONSENT_RESPONSE, consentResponseHT);
//				participantForm.setIsConsentAvailable(Constants.PARTICIPANT_CONSENT_EDIT_RESPONSE);
			}
		}
		/*
		 * Falguni Sachde bug id :8150 Set participantId as request attribute as
		 * Its required in case of view annotations view of Edit participant
		 */
		...
		if (participantForm.getOperation().equals(Constants.ADD))
		{
			...
			if (pageOf.equalsIgnoreCase(Constants.PAGE_OF_PARTICIPANT_CP_QUERY))
			{
				...
				if (collProtId != null)
				{
					...
					{
//						participantForm.setIsConsentAvailable(Constants.NO_CONSENTS_DEFINED);
						participantForm.setCollectionProtocolRegistrationValue(
...
//						participantForm.setIsConsentAvailable(Constants.PARTICIPANT_CONSENT_ENTER_RESPONSE);
		...
		setUserMessages(request);
		
		return mapping.findForward(pageOf);
			}

	private void setUserMessages(HttpServletRequest request) {		
		for (String msg : MessagesHolder.getErrorMessages())
			addErrorMessage(request, msg);
		for (String msg : MessagesHolder.getInformationalMessages())
			addInfoMessage(request, msg);
	...
	private void addErrorMessage(HttpServletRequest request,
		...
		ActionError msg = new ActionError("errors.item", message);
		errors.add("message", msg);
		request.setAttribute(Globals.ERROR_KEY, errors);
	}

	private void addInfoMessage(HttpServletRequest request, String txtMsg) {
		...
		ActionMessage msg = new ActionMessage("errors.item", txtMsg);
	...
	private void setConsentForParticipant(IBizLogic bizLogic, String participantId, Map<String, Object> collectionProtocolRegistrationValues, int counter) throws BizLogicException
	{
		final Collection cprColl = (Collection) bizLogic.retrieveAttribute(
				Participant.class.getName(), Long.parseLong(participantId),"elements(collectionProtocolRegistrationCollection)");
		if(collectionProtocolRegistrationValues != null && collectionProtocolRegistrationValues.size() > 0)
		{
			for(int i = 1; i <= counter; i++ )
			{
				final Iterator iter = cprColl.iterator();
				final String cpIdKey = "CollectionProtocolRegistration:" + i
				+ "_CollectionProtocol_id";
				String cpId = collectionProtocolRegistrationValues.get(cpIdKey).toString();
				while (iter.hasNext())
				{
					final CollectionProtocolRegistration cpr = (CollectionProtocolRegistration) iter.next();
					if(cpId.equals(cpr.getCollectionProtocol().getId().toString()))
					{
						final String isConsentAvailable = "CollectionProtocolRegistration:" + i
						+ "_isConsentAvailable";
						if(cpr.getCollectionProtocol().getConsentTierCollection() != null && cpr.getCollectionProtocol().getConsentTierCollection().isEmpty())
						{
