...
public class DeIDPipelineManager
{
	...
	private void processReports(List isprIDList) throws Exception
	{
		...
		if (isprIDList != null || !isprIDList.isEmpty())
		{
			...
			catch (final Exception ex)
			{
				...
				ex.printStackTrace();
				deidExecutor.shutdown();// shut down the thread pool manager
				throw ex;
			}
	...
	/**
	 * Get CSV Logger Setting done.
	 */
	private void getCSVLoggerSetting()
	{
			...
			throws Exception
	{
		...
		final IdentifiedSurgicalPathologyReport identifiedReport =
				(IdentifiedSurgicalPathologyReport) CaCoreAPIService.
					getObject(edu.wustl.catissuecore.domain.
						pathology.IdentifiedSurgicalPathologyReport.class,
		...
		participantList = CaCoreAPIService.executeQuery(hqlQuery,Participant.class);
		...
		if (participantList != null && !participantList.isEmpty())
		{
			...
			hqlQuery = "select elements(p.participantMedicalIdentifierCollection)"
					...
					+ participant.getId();
			final Collection<ParticipantMedicalIdentifier>
					participantMedicalIdentifierCollection = CaCoreAPIService.executeQuery(hqlQuery,ParticipantMedicalIdentifier.class);
		...
		final CollectionProtocolRegistration collectionProtocolRegistration =cprInstFact.createObject();
	 ...
	 *             deidentified report
	 */
	private List<Long> getReportIDList() throws Exception
	{
		...
		final List<Long> isprIDList = CaCoreAPIService.executeQuery(hqlQuery,Long.class);
		return isprIDList;
	}
