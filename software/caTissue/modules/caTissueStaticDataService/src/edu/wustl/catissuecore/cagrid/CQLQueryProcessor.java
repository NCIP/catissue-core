 ...
 */
public class CQLQueryProcessor extends
		...
		if (!isUseLocalApi()) {
			try {
				...
				throw new InitializationException(
						"Could not determine a remote API url: "
								+ ex.getMessage(), ex);
			}
		}
	}
	...
	public CQLQueryResults processQuery(CQLQuery cqlQuery) throws MalformedQueryException, QueryProcessingException {
		...
		try {
			...
			throw new QueryProcessingException(
					"Error checking query for default Attribute predicate values: "
							+ ex.getMessage(), ex);
		}
		...
		try {
			if (isUseGridIdentLogin()) {
				rawResults = WAPIUtility
						.getClient(getRemoteApplicationUrl(),
								getStaticLoginUser(), getStaticLoginPass())
						.getService().executeQuery(cqlQuery, getGridId());
			} else {
				rawResults = applicationService.query(cqlQuery);
			}
		} catch (ApplicationException ex) {
			String message = "Error processing CQL query in the caCORE ApplicationService: "
					+ ex.getMessage();
			LOG.error(message, ex);
			throw new QueryProcessingException(message, ex);
		} catch (Exception ex) {
			String message = "Error getting caCore Application Service: "
					+ ex.getMessage();
			LOG.error(message, ex);
		}
		...
		if (cqlQuery.getQueryModifier() != null) {
			...
			cqlResults = CQLResultsCreationUtil.createAttributeResults(
					resultsAsArrays, cqlQuery.getTarget().getName(),
					attributeNames);

		} else if (isCountOnly) {
			cqlResults = CQLResultsCreationUtil.createCountResults(
					rawResults.size(), cqlQuery.getTarget().getName());
			...
			try {
				...
				throw new QueryProcessingException(
						"Error loading class to QName mappings: "
								+ ex.getMessage(), ex);
			}
			try {
				cqlResults = CQLResultsCreationUtil.createObjectResults(
						rawResults, cqlQuery.getTarget().getName(),
						classToQname);
			} catch (ResultsCreationException ex) {
				throw new QueryProcessingException(
						"Error packaging query results: " + ex.getMessage(), ex);
			}
		}
		return cqlResults;
	}
