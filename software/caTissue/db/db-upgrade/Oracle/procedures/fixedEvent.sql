create or replace
procedure Fixed_Event_migrate(event_name in varchar2) IS
 ...
 -----------------------------------calling function--------------------------------------------------------------- */       
  ...
  ------------------------------------------------------------------------------------------------------------------       
     ...
     --DBMS_OUTPUT.PUT_LINE(specimen_id||'  '||specimen_event_identifier||'  '||seqval);
      EXECUTE IMMEDIATE query_text using dispo_DUR_min, dispo_FIXATION_TYPE,specimen_event_identifier, seqval; 
