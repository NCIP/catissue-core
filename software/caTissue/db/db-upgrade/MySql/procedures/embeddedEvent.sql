drop procedure if exists   Embedded_call_parameter;
//
CREATE  PROCEDURE   Embedded_call_parameter()
Begin
  ...
  declare dispo_EMBEDDING_MEDIUM Text;
  declare query_text Text;
  declare query_text_form Text;
  declare event_name varchar (100) default 'EmbeddedEventParameters';
  #-----------------------@using parameter-----------------------------
 ...
 #-------------------------------------------------------------------
  declare mig_cursor  cursor for 
    select spec.identifier,
           ...
           spec.user_id,
           spec.comments,
           dispo.EMBEDDING_MEDIUM,
		    absspec.parent_specimen_id
      from   catissue_embedded_event_param dispo,
        catissue_specimen_event_param spec,
        catissue_specimen se,
		 catissue_abstract_specimen absspec
	where
      dispo.identifier = spec.identifier and spec.specimen_id=se.identifier and absspec.IDENTIFIER = se.identifier;
  ...
  #------------------------------------------------------------------------------------------------------------------       
      ...
      select count(*) into flag  from 
        catissue_embedded_event_param emb,
        catissue_specimen_event_param spec
        where 
   ...
   set @des_reason := dispo_EMBEDDING_MEDIUM;
   set @s_seq_var :=seq_ver;
   execute stmt using @des_reason,@sp_id,@s_seq_var;   
