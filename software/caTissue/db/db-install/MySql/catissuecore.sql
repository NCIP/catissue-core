...
create table CATISSUE_CONTAINER_TYPE (
   ...
   COMMENTS text,
   ACTIVITY_STATUS VARCHAR(50),
   ONE_DIMENSION_LABELLING_SCHEME  VARCHAR(255),
   TWO_DIMENSION_LABELLING_SCHEME  VARCHAR(255),
   primary key (IDENTIFIER)
);
...
create table CATISSUE_ABSTRACT_POSITION (
	...
	POSITION_DIMENSION_ONE INTEGER,
	POSITION_DIMENSION_TWO INTEGER,
	POSITION_DIMENSION_ONE_STRING VARCHAR(255),
	POSITION_DIMENSION_TWO_STRING VARCHAR(255),
	primary key (IDENTIFIER)
...
CREATE TABLE `csm_migrate_user` (
  ...
  UNIQUE KEY `TARGET_IDP_NAME` (`TARGET_IDP_NAME`,`MIGRATED_LOGIN_NAME`)
);
...
create table catissue_default_action (IDENTIFIER bigint NOT NULL, PRIMARY KEY (IDENTIFIER));
