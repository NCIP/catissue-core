...
<script>
		...
		function showConflictSpecimenWindow(conflictingSpecimens){
			...
			+"<div  style='float:left;width:25%;'><div class='black_ar_b'><spna>Label</span></div>"
				+"<div  class='aliquot_details aliquot_form_label'><span>"+conflictingSpecimens[0].label+"</span></div>"
		...
		function init_grid(){		
			...
			aliquotGrid.setColTypes("ed,ed,ed,ed,ed,ed,ro");
			if("${isLabelGenerationOn}"=="true" || "${isBarGenerationOn}"=="true"){
		...
		//	aliquotGrid.setColSorting("str,str,str,str,str,str,str");
			...
			aliquotGrid.attachEvent("onEditCell", function(stage,rId,cInd,nValue,oValue){
				if(stage==2 ){
					...
					aliquotGrid.cellById(rId,4).setValue("");
					aliquotGrid.cellById(rId,5).setValue("");
					}
				}
				return true;
		...
		function onGridLoadSetAutoGenerated(grid_obj,rowCount){
			if("${isLabelGenerationOn}"=="true" || "${isBarGenerationOn}"=="true"){
				for(var cnt=1;cnt<=grid_obj.getRowsNum();cnt++){
					grid_obj.cellById(cnt,0).setValue("Auto Generated");
					grid_obj.cellById(cnt,1).setValue("Auto Generated");
				}
			}
		}
		function onGridLoad(grid_obj,rowCount){
			for(var cnt=1;cnt<=grid_obj.getRowsNum();cnt++){
				grid_obj.cellById(cnt,6).setValue("<a href='#' onClick='openMapPopUp(\""+cnt+"\")' >Map</a>");
			}
		}
		function openMapPopUp(rowNum){
			aliquotGrid.selectRow(rowNum);
			var containerName = aliquotGrid.cellById(rowNum,3).getValue();
			var pos1 = aliquotGrid.cellById(rowNum,4).getValue();
			var pos2 = aliquotGrid.cellById(rowNum,5).getValue();
		...
		function setNewStoragePositionForAliquot(rowNum,containerName,pos1,pos2){
			aliquotGrid.cellById(rowNum,3).setValue(containerName);
			aliquotGrid.cellById(rowNum,4).setValue(pos1);
			aliquotGrid.cellById(rowNum,5).setValue(pos2);
		}
		...
		function submitAliquot(){
			...
			var loader=dhtmlxAjax.postSync("CreateAliquotsFromSpecimen.do","dataJSON="+JSON.stringify(tabDataJSON));
			...
			if(loader.xmlDoc.responseText != null)
			{
				...
				if(response.success)
				{
					...
					nodeId= "Specimen_"+response.parentSpecimenId;
					aliquotGrid.clearAll();
					aliquotGrid.loadXMLString(response.aliquotGridXml);
					onGridLoad(aliquotGrid);
					document.getElementById("ali_currentAvailableQuantity").innerHTML = response.currentAvailableQuantity+" "+ response.unit;;
					
					document.getElementById('assignTargetCall').value = "giveCall('AssignTagAction.do?entityTag=SpecimenListTag&entityTagItem=SpecimenListTagItem&objChkBoxString="+response.aliquotIdList+"','Select at least one existing list or create a new list.','No specimen has been selected to assign.','"+response.aliquotIdList+"')";
...
function onResubmit(validated,label){
	...
	var loader=dhtmlxAjax.postSync("GetAliquotInfoAjax.do",'validated='+validated+'&searchBasedOn='+searchBasedOn+'&parentSpecimentLabel='+specimenLabel+"&aliquotCount="+aliquotCount+"&quantityPerAliquot="+quantityPerAliquot);
...
function applyFirstToALL(){
	var contName = aliquotGrid.cellById(1,3).getValue();
	for(var cnt=2;cnt<=aliquotGrid.getRowsNum();cnt++){
		aliquotGrid.cellById(cnt,3).setValue(contName);
		aliquotGrid.cellById(cnt,4).setValue("");
		aliquotGrid.cellById(cnt,5).setValue("");
	}
}
function getCommaseparatedList(){
	...
	for(var cnt=1;cnt<=aliquotGrid.getRowsNum();cnt++){
		...
		label = label+aliquotGrid.cellById(cnt,0).getValue();
	}
	return label;
}
