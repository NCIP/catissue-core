...
var request = newXMLHTTPReq();
			...
			request.open("GET","rest/specimens/"+specimenLabel+"/aliquots/"+JSON.stringify(paramStr),true);
			request.setRequestHeader("Content-Type","application/json");
			request.send();
...
function loadAliquotGridWithNewData(aliquotGrid,aliquotDetailsDTO){
	...
	for(var cnt = 0; cnt < gridData.length; cnt++){
		aliquotGrid.addRow(newId+cnt,gridData[cnt].aliqoutLabel+","+gridData[cnt].barCode+","+gridData[cnt].quantity+","+gridData[cnt].storagecontainer+","+gridData[cnt].pos1+","+gridData[cnt].pos2+",,"+gridData[cnt].aliquotId,cnt+1);
	}
}
