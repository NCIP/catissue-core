package clinical_annotation.service.globus.resource;
...
import clinical_annotation.common.CAConstants;
import clinical_annotation.stubs.CAResourceProperties;
import clinical_annotation.service.CAConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class CAResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CAResourceBase.class);

	private CAResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.dataservice.DomainModel getDomainModel(){
		return ((CAResourceProperties) getResourceBean()).getDomainModel();
	}
	
	public void setDomainModel(gov.nih.nci.cagrid.metadata.dataservice.DomainModel domainModel ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CAConstants.DOMAINMODEL);
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CAResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CAConstants.SERVICEMETADATA);
	...
	public CAResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (CAResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadDomainModelFromFile() {
      if(getDomainModel()==null){
		try {
			...
			((CAResourceProperties) this.getResourceBean()).setDomainModel((gov.nih.nci.cagrid.metadata.dataservice.DomainModel) Utils.deserializeDocument(dataFile.getAbsolutePath(),
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CAResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
