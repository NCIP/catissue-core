...
ALTER TABLE CATISSUE_USER ADD IS_ADMIN BIT DEFAULT 0;

CREATE TABLE TEMP_CATISSUE_USER SELECT * FROM CATISSUE_USER;

UPDATE CATISSUE_USER SET IS_ADMIN = 1 WHERE IDENTIFIER IN 
(SELECT IDENTIFIER FROM TEMP_CATISSUE_USER WHERE CSM_USER_ID IN 
(SELECT USER_ID FROM CSM_USER_GROUP WHERE GROUP_ID = 1));

DROP TABLE TEMP_CATISSUE_USER;
