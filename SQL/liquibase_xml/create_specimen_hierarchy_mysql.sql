...
begin
 ...
 DECLARE curs CURSOR FOR SELECT IDENTIFIER FROM CATISSUE_SPECIMEN WHERE PARENT_SPECIMEN_ID IS NULL;
 ...
 DROP TABLE IF EXISTS CATISSUE_SPECIMEN_HIERARCHY;
 CREATE TABLE CATISSUE_SPECIMEN_HIERARCHY(ANCESTOR_ID BIGINT NOT NULL, DESCENDENT_ID BIGINT NOT NULL);
 ...
 WHILE NOT endOfTable DO
  ...
  IF LENGTH(@sql_store) > 0 THEN
   SET @sql_stmt = CONCAT( 'INSERT INTO CATISSUE_SPECIMEN_HIERARCHY(ANCESTOR_ID, DESCENDENT_ID) VALUES ' , @sql_store );
...
begin
 ...
 DECLARE curs CURSOR FOR SELECT IDENTIFIER FROM CATISSUE_SPECIMEN WHERE PARENT_SPECIMEN_ID = CONVERT(nodeId, UNSIGNED INTEGER);
...
ALTER TABLE CATISSUE_SPECIMEN_HIERARCHY ADD CONSTRAINT UNIQUE(ANCESTOR_ID, DESCENDENT_ID)//

ALTER TABLE CATISSUE_SPECIMEN_HIERARCHY ADD CONSTRAINT FOREIGN KEY(ANCESTOR_ID) REFERENCES CATISSUE_SPECIMEN(IDENTIFIER)//
ALTER TABLE CATISSUE_SPECIMEN_HIERARCHY ADD CONSTRAINT FOREIGN KEY(DESCENDENT_ID) REFERENCES CATISSUE_SPECIMEN(IDENTIFIER)//



DROP TRIGGER IF EXISTS TRG_SPECIMEN_REPLICATION //

CREATE TRIGGER TRG_SPECIMEN_REPLICATION AFTER INSERT 
