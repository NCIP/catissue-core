...
import edu.wustl.catissuecore.api.test.ExcelTestCaseUtility;
...
public class SpecimenCollectGroupBizTestCases extends CaTissueSuiteBaseTest
{
	...
	{ 
		...
		specimenCollectionGroup.setBarcode(((SpecimenCollectionGroup) TestCaseUtility.getObjectMap(SpecimenCollectionGroup.class)).getBarcode());
		 try {
			 String query = "from edu.wustl.catissuecore.domain.SpecimenCollectionGroup as specimenCollectionGroup where "
					+ "specimenCollectionGroup.barcode= '"+ ((SpecimenCollectionGroup) TestCaseUtility.getObjectMap(SpecimenCollectionGroup.class)).getBarcode()+"'";	
        	 List resultList = appService.search(query);
        	 if(resultList!=null)
        	 {
//        	
