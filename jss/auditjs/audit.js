 ...
 */
function onAuditPageLoad(datePttrn) {
	getUsers();
	getObjects();
	getEvents();
	initChosen()
	initDatePicker(datePttrn);
}
/*
...
function initChosen() {
	...
	jQuery('#object_type').chosen({
		"search_contains" : true
	});
	jQuery('#operation').chosen({
		"search_contains" : true
	});
}

function initDatePicker(datePttrn) {
	$('#datefrom , #dateto').datepicker({
		changeMonth : true,
		changeYear : true,
		dateFormat : datePttrn
	});
...
function exportData() {
	try {
		...
		if (jQuery.isEmptyObject(jsonObject)) {
			...
			window.open("rest/ng/auditreport?json="
