...
openspecimen.ui.fancy.Users = function() {
  var baseUrl = 'http://localhost:9090/openspecimen/rest/ng/users/';
  ...
  this.getUsers = function(queryTerm, callback) {
    ...
    var xhr;
    if (queryTerm) {
      xhr = userSvc.query({searchString: queryTerm});
  ...
  this.getUser = function(userId, callback) {
    ...
    var userSvc = angular.element($("#de-form")).injector().get('User');
    userSvc.getById(userId).then(
      function(user) {
        var result = {id: user.id, text: user.lastName + ', ' + user.firstName};
