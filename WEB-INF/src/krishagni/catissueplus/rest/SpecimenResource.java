...
public class SpecimenResource
{
	...
	private static final Logger LOGGER = Logger
			.getCommonLogger(SpecimenResource.class);

	@Path("/{label}/derivatives")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public Response createDerive(@PathParam("label") String label,
			String derivedDetails)
	{
		Gson gson = AppUtility.initGSONBuilder().create();
		Response response = null;
		try
		{
			DerivedDTO derivedDTO = gson.fromJson(derivedDetails,
					DerivedDTO.class);
			SpecimenHandler specimenHandler = new SpecimenHandler();
			SpecimenDTO specimenDTO = specimenHandler.createDerivative(
					derivedDTO, getSessionDataBean());
			response = Response.status(Status.CREATED.getStatusCode())
					.entity(specimenDTO).type(MediaType.APPLICATION_JSON)
					.build();
		}
		...
		catch (Exception e)
		{
			...
			response = Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity(e.getMessage()).header("errorMsg", e.getMessage())
	...
	public Response getSpecimenDetails(@PathParam("label") String label)
	{
		...
		{
			SpecimenHandler specimenHandler = new SpecimenHandler();
			SpecimenDTO specimenDTO = specimenHandler.getSpecimenDetails(label,
					getSessionDataBean());
			response = Response.ok(specimenDTO, MediaType.APPLICATION_JSON)
					.build();
		}
		catch (CatissueException e)
		{
	...
	public Response createSpecimen(String specimenDetails)
	{
		...
		{
			Gson gson = AppUtility.initGSONBuilder().create();
			SpecimenDTO specimenDTO = gson.fromJson(specimenDetails,
					SpecimenDTO.class);
			SpecimenHandler specimenHandler = new SpecimenHandler();
			specimenDTO = specimenHandler.createSpecimen(specimenDTO,
					getSessionDataBean());
			response = Response.status(Status.CREATED.getStatusCode())
					.entity(specimenDTO).type(MediaType.APPLICATION_JSON)
					.build();
	...
	public Response updateSpecimen(String specimenDetails)
	{
		...
		try
		{
			SpecimenDTO specimenDTO = gson.fromJson(specimenDetails,
					SpecimenDTO.class);
			SpecimenHandler specimenHandler = new SpecimenHandler();
			specimenDTO = specimenHandler.updateSpecimen(specimenDTO,
					getSessionDataBean());
	...
	{
		SpecimenErrorCodeEnum errorCodeEnum = SpecimenErrorCodeEnum.getStatus(e
				.getErrorCode());
