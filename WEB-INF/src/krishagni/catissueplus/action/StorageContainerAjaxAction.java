...
public class StorageContainerAjaxAction extends DispatchAction
{
			...
			IOException, JSONException
	{
		...
		storageContainerViewDTO = storageContainerBizlogic.getContainerDetailsForDataView(
				containerName, dao);//get the container details DTO
		...
		for (int i = 0; i < storageContainerViewDTO.getOneDimensionCapacity(); i++)
		{
			...
			dimensionOneLabels.put(
					String.valueOf(i + 1),
					AppUtility.getPositionValue(
							storageContainerViewDTO.getOneDimensionLabellingScheme(), i + 1));
		}

		for (int i = 0; i < storageContainerViewDTO.getTwoDimensionCapacity(); i++)
		{
			//DimensionOne label with respective dimension label scheming
			dimensionTwoLabels.put(
					String.valueOf(i + 1),
					AppUtility.getPositionValue(
							storageContainerViewDTO.getTwoDimensionLabellingScheme(), i + 1));
		}

		JSONObject returnedJObject = new JSONObject();
		returnedJObject.put(Constants.POS1_CONTROL_NAME,
				request.getSession().getAttribute(Constants.POS1));
		returnedJObject.put(Constants.POS2_CONTROL_NAME,
				request.getSession().getAttribute(Constants.POS2));
		returnedJObject.put(Constants.CONTROL_NAME,
				request.getSession().getAttribute(Constants.CONTROL_NAME));
