...
public class CatissueNotificationServiceImpl implements CatissueNotificationService {
	...
	public ResponseEvent<CollectionProtocolRegistrationDetail> registerParticipant(RequestEvent<NotifiedRegistrationDetail> req) {
		try {
			NotifiedRegistrationDetail detail = req.getPayload();
			
			CPStudyMappingDao cpStudyMappingdao = daoFactory.getCPStudyMappingDao();
			Long cpId = cpStudyMappingdao.getMappedCPId(detail.getAppName(), detail.getStudyId());
			if (cpId == null) {
				return ResponseEvent.userError(CPStudyMappingErrorCode.STUDY_NOT_MAPPED_WITH_CP);
			...
			cprDetail.setRegistrationDate(detail.getEnrollmentDate());
			cprDetail.setCpId(cpId);
			
			RequestEvent<CollectionProtocolRegistrationDetail> registerReq = new RequestEvent<CollectionProtocolRegistrationDetail>();
			registerReq.setPayload(cprDetail);
			registerReq.setSessionDataBean(req.getSessionDataBean());

			return cprSvc.createRegistration(registerReq);
