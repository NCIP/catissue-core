...
public class SimilarContainersAction extends SecureAction
{
			...
			HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			request.setAttribute("storageListForTransferEvent",
			...
			final IBizLogic ibizLogic = factory
					.getBizLogic(Constants.DEFAULT_BIZ_LOGIC);

			// request =
			//AppUtility.setCollectionProtocolList(request,similarContainersForm
			// .getSiteId());
			if ("Site".equals(similarContainersForm
					.getParentContainerSelected())) {
				request = AppUtility.setCollectionProtocolList(request,
						similarContainersForm.getSiteId(),dao);
			} else if ("Auto".equals(similarContainersForm
					.getParentContainerSelected())) {
				final long parentContId = similarContainersForm
			...
			} else if ("Manual".equals(similarContainersForm
					.getParentContainerSelected())) {
				final String selectedContName = similarContainersForm
						.getSelectedContainerName();

				final Site site = bizLogic
						.getRelatedSiteForManual(selectedContName,dao);
				if (site != null) {
					request = AppUtility.setCollectionProtocolList(request,
							site.getId(),dao);
				} else {
			...
			final List storageTypeListWithAny = AppUtility.getStorageTypeList(
					list2, true);
			request.setAttribute(Constants.HOLDS_LIST1, storageTypeListWithAny);

			final List storagetypeList = new ArrayList();
			NameValueBean nvb = new NameValueBean(similarContainersForm
					.getTypeName(), new Long(similarContainersForm.getTypeId()));
			...
			list = ibizLogic.getList(Site.class.getName(), siteDisplayField,
					valueField, true);
			...
			if (sessionDataBean != null && !sessionDataBean.isAdmin()) {
				final Set<Long> idSet = new UserBizLogic()
			...
			final List specimenClassTypeList = AppUtility
					.getSpecimenClassTypeListWithAny();
			request.setAttribute(Constants.HOLDS_LIST2, specimenClassTypeList);

			// Gets the Specimen array Type List and sets it in request
			final List list3 = dao.retrieve(SpecimenArrayType.class
					.getName());
			final List spArrayTypeList = AppUtility
					.getSpecimenArrayTypeList(list3);
			...
			final Object object = dao.retrieveById(StorageType.class
					.getName(), new Long(similarContainersForm.getTypeId()));
			...
			similarContainersForm.setOneDimensionLabel(storageType
					.getOneDimensionLabel());
			similarContainersForm.setTwoDimensionLabel(storageType
			...
			if (Constants.SITE.equals(selectedParentContainer)) {
				if (siteId != -1) {
					final Object siteObject = dao.retrieveById(Site.class
							.getName(), new Long(siteId));
					if (siteObject != null) {
			...
			// Suman: for bug 8904
			else if (Constants.STORAGE_TYPE_POSITION_MANUAL
					.equals(selectedParentContainer)) {
				this.logger.debug("Long.parseLong(request.getParameter"
						+ "(parentContainerId)..."
						+ request.getParameter("parentContainerId"));
				this.logger
						.debug("similarContainerForm.getTypeId()......................."
				...
				// if (similarContainersForm.getParentContainerId() == 0)
				{
					...
					final String containerName = similarContainersForm
							.getSelectedContainerName();
					...
					List l = dao.retrieve(sourceObjectName, selectColumnName, queryWhereClause);
					
					final List containerIdList = bizLogic.retrieve(
							sourceObjectName, selectColumnName,
							whereColumnName, whereColumnCondition,
							whereColumnValue, joinCondition);

					if (!containerIdList.isEmpty()) {
						similarContainersForm
								.setParentContainerId(((Long) containerIdList
										.get(0)).longValue());

						boolean isContainerFull = false;
						/**
						 * Following code is added to set the x and y dimension
						 * in case only storage container is given and x and y
						...
						if (similarContainersForm.getPos1() == null
							...
							container.setId(similarContainersForm
									.getParentContainerId());
							Position position = bizLogic
									.getFirstAvailablePositionInContainer(
											container, dao);
							if (position != null) {
								similarContainersForm.setPos1(position
										.getXPos()
										+ "");
								similarContainersForm.setPos2(position
										.getYPos()
										+ "");
				...
				if (parentContId != null) {
					final Object containerObject = dao.retrieveById(
							StorageContainer.class.getName(), new Long(
									parentContId));
			...
			final TreeMap containerMap = bizLogic
					.getAllocatedContaienrMapForContainer(new Long(request
							.getParameter("typeId")).longValue(),
							exceedingMaxLimit, similarContainersForm
									.getSelectedContainerName(),
							sessionDataBean,dao);
			...
			final int noOfContainers = Integer.parseInt(request
					.getParameter("noOfContainers"));
			if (similarContainersForm.getSimilarContainersMap().size() == 0) {
				...
				similarContainersForm.setSimilarContainerMapValue(
						"checkedButton", Integer.toString(siteOrParentCont));
				...
				if (!Constants.SITE.equals(selectedParentContainer)) {
					...
					startingPoints[0] = Long.toString(similarContainersForm
							.getParentContainerId());
					...
					if (!Constants.SITE.equals(selectedParentContainer)
							&& !(this.checkAvailability(containerMap,
									noOfContainers))) {
						ActionErrors errors = (ActionErrors) request
								.getAttribute(Globals.ERROR_KEY);
						if (errors == null) {
				...
				for (int i = 1; i <= noOfContainers; i++) {
					...
					if (Constants.SITE.equals(selectedParentContainer)) {
						similarContainersForm.setSimilarContainerMapValue(
								"simCont:" + i + "_siteId", new Long(siteId)
										.toString());
						similarContainersForm.setSimilarContainerMapValue(
								"simCont:" + i + "_siteName", maxSiteName
										.toString());
					}

				}

				final String contName = similarContainersForm
						.getContainerName();
				final String barcode = similarContainersForm.getBarcode();

				this.logger.debug("contName " + contName + " barcode "
						+ barcode + " <<<<---");
				similarContainersForm.setSimilarContainerMapValue(
						"simCont:1_name", contName);
				similarContainersForm.setSimilarContainerMapValue(
						"simCont:1_barcode", barcode);
			...
			if (change != null && !change.equals("")) {
				...
				if (Constants.SITE.equals(selectedParentContainer)) {
					final String Id = (String) similarContainersForm
							.getSimilarContainerMapValue("simCont:" + i
									+ "_siteId");
					final Object siteObject2 = dao.retrieveById(Site.class
							.getName(), new Long(Id));
					if (siteObject2 != null) {
