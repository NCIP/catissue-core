...
public class ShowShipmentReceivingAction extends SecureAction
{
			...
			HttpServletResponse response) throws Exception
	{
		...
		{
			shipmentId=Long.parseLong(request.getParameter(edu.wustl.catissuecore.util.global.Constants.SYSTEM_IDENTIFIER));
		}
		ShipmentReceivingForm shipmentReceivingForm =(ShipmentReceivingForm)form;
		Shipment shipment=null;
		// Get the Shipment details.
		IFactory factory = AbstractFactoryConfig.getInstance().getBizLogicFactory();
		ShipmentBizLogic shipmentBizLogic = (ShipmentBizLogic)factory.getBizLogic(Constants.SHIPMENT_FORM_ID);
		...
		StorageContainerBizLogic bizLogic = (StorageContainerBizLogic) factory.getBizLogic(edu.wustl.catissuecore.util.global.Constants.STORAGE_CONTAINER_FORM_ID);
		// Get storageType for a "Shipment container".
		StorageType st = ((List<StorageType>) bizLogic.retrieve(StorageType.class.getName(), Constants.NAME, Constants.SHIPMENT_CONTAINER_TYPE_NAME)).get(0);
		...
		*/
		...
		setSpecimenStorageInformation(bizLogic,containerMap,request,shipmentReceivingForm,shipment);
		...
		List list = bizLogic.getSiteList(siteDisplayField,valueField,
					activityStatusArray,sessionDataBean.getUserId());
	 ...
	 */
	private void setContainerStorageInformation(TreeMap containerMap, StorageContainerBizLogic bizLogic,HttpServletRequest request, ShipmentReceivingForm shipmentReceivingForm)
	{
	 ...
	 * @throws DAOException if some database operation fails
	 */
	private void setSpecimenStorageInformation(StorageContainerBizLogic bizLogic, TreeMap containerMap, HttpServletRequest request, ShipmentReceivingForm shipmentReceivingForm, Shipment shipment)
	{
//		For Storage Location Allocation
		...
		String operation = (String) request.getParameter(edu.wustl.catissuecore.util.global.Constants.OPERATION);
				...
				&& stContSelection!=null)
		{
			if(stContSelection.equals("3"))
			{
				...
				if(specimenId!=null && !specimenId.trim().equals(""))
				{
					...
					IFactory factory = AbstractFactoryConfig.getInstance().getBizLogicFactory();
					NewSpecimenBizLogic newSpecimenBizLogic=(NewSpecimenBizLogic) factory.getBizLogic(edu.wustl.catissuecore.util.global.Constants.NEW_SPECIMEN_FORM_ID);
				...
				request.setAttribute(edu.wustl.catissuecore.util.global.Constants.COLLECTION_PROTOCOL_ID,collectionProtocolId);
			}
		}
//		else
//		{
//		}
		if(specimenId!=null && !specimenId.trim().equals(""))
		{
			request.setAttribute(edu.wustl.catissuecore.util.global.Constants.SPECIMEN_ID, specimenId);
		}
	 ...
	 */
	private void checkForSufficientAvailablePositions(HttpServletRequest request,
			Map containerMap, int storableCount, String objectName)
	{
		// Commented due to removing 'auto' functionality.
		/*
		int counter = 0;
	...
	private List getStoragePositionTypeList()
	{
		...
		storagePositionTypeList.add(new NameValueBean(edu.wustl.catissuecore.util.global.Constants.STORAGE_TYPE_POSITION_VIRTUAL,
							edu.wustl.catissuecore.util.global.Constants.STORAGE_TYPE_POSITION_VIRTUAL_VALUE));
		storagePositionTypeList.add(new NameValueBean(edu.wustl.catissuecore.util.global.Constants.STORAGE_TYPE_POSITION_MANUAL,
