	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if (applet instanceof SpecimenArrayApplet) 
		{
			...
			{	
			  ...
			  {	
				concKey = SpecimenArrayAppletUtil.getArrayMapKey(rows[i],columns[j],model.getColumnCount(),AppletConstants.ARRAY_CONTENT_ATTR_CONC_INDEX);
				modelMap.put(concKey,conc);
				quantityKey = SpecimenArrayAppletUtil.getArrayMapKey(rows[i],columns[j],model.getColumnCount(),AppletConstants.ARRAY_CONTENT_ATTR_QUANTITY_INDEX);
				modelMap.put(quantityKey,quantity);
/*				System.out.println(" rows [" + i + "] :: " + rows[i]);
				System.out.println(" columns [" + i + "] :: " + columns[j]);
*/			}
		  }	  
		}
	}
