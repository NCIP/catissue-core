...
public class ClinicalStudyForm extends SpecimenProtocolForm
{
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request)
	{
		...
		try
		{
			...
			{
				errors.add(ActionErrors.GLOBAL_ERROR, new ActionError(ERROR_ITEM_REQD,
			...
			while (iter.hasNext())
			{
				...
				if (key.indexOf("studyCalendarEventPoint") != -1 && validator.isEmpty(value))
				{
					errors.add(ActionErrors.GLOBAL_ERROR,
							new ActionError(ERROR_ITEM_REQD, ApplicationProperties
				...
				if (key.indexOf("noOfEntries") != -1)
				{
					...
					{
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError(ERROR_ITEM_REQD,
								ApplicationProperties.getValue("clinicalStudyEvent.noOfEntries")));
					}
					else
					{
						if ("#".equals(value))
				...
				if (key.indexOf("studyFormId") != -1 && !validator.isValidOption(value))
				{
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("errors.item.selected",
							ApplicationProperties.getValue("clinicalStudyEvent.studyForm")));
				}

				if (key.indexOf("studyFormLabel") != -1 && validator.isEmpty(value))
				{
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError(ERROR_ITEM_REQD,
							ApplicationProperties.getValue("clinicalStudyEvent.studyFormLabel")));
				}
