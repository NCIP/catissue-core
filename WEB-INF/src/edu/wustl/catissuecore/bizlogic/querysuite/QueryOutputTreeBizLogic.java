...
public String updateTreeForLabelNode(String nodeId, Map<Long, OutputTreeDataNode> idNodeMap, Map<Long, Map<AttributeInterface, String>> columnMap,
	...
	String selectSql = "";
	int index = -1;
	String actualParentNodeId = nodeId.substring(0, nodeId.indexOf(Constants.NODE_SEPARATOR));
	...
	if (!actualParentNodeId.equalsIgnoreCase(Constants.NULL_ID))
	{
		...
		selectSql = selectSql + " from " + tableName + " where (" + parentIdColumnName + " = '" + nodeIds[1] + "' and "+idColumnOfCurrentNode+" is not null)";
	}
	if (actualParentNodeId.equalsIgnoreCase(Constants.NULL_ID))
	{
