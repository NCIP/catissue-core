...
public class SummaryBizLogic extends CatissueDefaultBizLogic
{
	...
	private List getUserData() throws DAOException, ClassNotFoundException
	{
		...
		String sql = "SELECT concat(FIRST_NAME,concat(', ',LAST_NAME)), B.PHONE_NUMBER FROM CATISSUE_USER A, CATISSUE_ADDRESS B WHERE CSM_USER_ID  IN (SELECT USER_ID FROM CSM_USER_GROUP WHERE GROUP_ID=1) AND A.IDENTIFIER>1 AND A.ADDRESS_ID=B.IDENTIFIER  AND A"
					+ DISABLED;
		try
		{
