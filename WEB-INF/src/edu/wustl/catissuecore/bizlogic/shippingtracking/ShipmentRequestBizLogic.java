...
{
	Logger logger = Logger.getCommonLogger(ShipmentRequestBizLogic.class);
	/**
	...
	public boolean validate(Object obj, DAO dao, String operation) throws BizLogicException
	{
		...
		if (obj instanceof ShipmentRequest == false)
		{
			...
			throw new BizLogicException(ErrorKey.getErrorKey("errors.invalid.object.passed"),null,"object is not the instance of ShipMent request class");
		}
		...
		if (shipmentRequest == null)
		{
			...
			throw new BizLogicException(ErrorKey.getErrorKey("errors.domain.object.null"),null,"shipment object is null");
		}
		...
		*/
		try
		{
			if (!shipmentRequest.isRequestProcessed()
				&& (shipmentRequest.getActivityStatus()!=null
						&& !shipmentRequest.getActivityStatus()
						.equals(Constants.ACTIVITY_STATUS_REJECTED)))
			{
				...
				{
					Iterator<Specimen> specimenIterator=shipmentRequest.getSpecimenCollection().iterator();
					while(specimenIterator.hasNext())
					{
						...
						if(specimen!=null)
						{
							...
							if(specimen!=null)
							{
								isValid=specimenBelongsToSite(specimen,shipmentRequest.getSenderSite().getId());
	...
	private boolean specimenBelongsToInTransitSite(Specimen specimen)
	{
		...
		{
			if(specimen.getSpecimenPosition().getStorageContainer().getName()
	...
	private boolean specimenBelongsToSite(Specimen specimen, Long id)
	{
		...
		if(specimen!=null)
		{
			...
			{
				if(specimen.getSpecimenPosition().getStorageContainer()!=null && specimen.getSpecimenPosition().getStorageContainer().getSite()!=null && specimen.getSpecimenPosition().getStorageContainer().getSite().getId().equals(id))
				{
					belongs=true;
				}
			}
	...
	protected void insert(Object obj, DAO dao, SessionDataBean sessionDataBean) throws BizLogicException
	{
		try
		{
			if (!(obj instanceof ShipmentRequest))
			{
				//throw new DAOException(ApplicationProperties.getValue("errors.invalid.object.passed"));
				logger.debug("invalid object passed");
				throw new BizLogicException(ErrorKey.getErrorKey("errors.invalid.object.passed"),null,"Invalid object passed");
			}
			ShipmentRequest shipmentRequest = (ShipmentRequest) obj;
			...
			if (!shipmentRequest.getActivityStatus().equals(Constants.ACTIVITY_STATUS_DRAFTED))
			{
						...
						&& shipmentRequest.getContainerCollection().size()>0)
				{
					receiverSiteId=((StorageContainer)shipmentRequest.getContainerCollection()
						...
						&& shipmentRequest.getSpecimenCollection().size()>0)
				{
					receiverSiteId=((Specimen)shipmentRequest.getSpecimenCollection().iterator().next()).getSpecimenPosition().getStorageContainer().getSite().getId();
				}
				Site site=null;
			...
			Collection<Specimen> specimenCollection=updateSpecimenCollection(
...
//			throw new DAOException(bizLogicException.getMessage());
	 ...
	 */
	private Collection<Specimen> updateSpecimenCollection(Collection<Specimen> specimenCollection, DAO dao,
	 ...
	 * @throws DAOException
	 */
	public Collection<ShipmentRequest> createRequestObjects(ShipmentRequest shipmentRequest, DAO dao,
			String operation) throws DAOException
	{
		...
		if(!(siteMap.isEmpty()))
		{
			...
			for (Long siteId : siteMap.keySet())
			{
				...
				{
					Specimen specimen=specimenIterator.next();
					if(specimen!=null && specimen.getSpecimenPosition()!=null && specimen.getSpecimenPosition().getStorageContainer()!=null)
					{
	 ...
	 */
	private ShipmentRequest createShipmentRequest(ShipmentRequest shipmentRequest, Collection<StorageContainer> containerCollection,Collection<Specimen> specimenCollection, Site site)
	{
	 ...
	 * @throws DAOException if some database operation fails.
	 */
	private Map<Long, Site> getUniqueSiteMap(ShipmentRequest shipmentRequest, DAO dao)throws DAOException
	{
	 ...
	 */
	public List<Object[]> getShipmentRequests(String selectColumnName,String columnName,String orderByField,Long[] siteId,int startIndex,int numOfRecords, String additionalWhereClause) throws BizLogicException
	{
	...
	public ShipmentRequest getShipmentRequestObject(long identifier) throws BizLogicException
	{
        ...
        while(containerIterator.hasNext())
        {
        	...
        	while(spPosIterator.hasNext())
        	{
        		...
        		Specimen specimen=(Specimen)retrieveAttribute(SpecimenPosition.class.getName(),specimenPosition.getId(), "specimen");
			...
			int startIndex,int numOfRecords) throws BizLogicException
	{
		...
		int count=getShipmentsCount(ShipmentRequest.class.getName(),whereClause,siteId,orderByField,startIndex,numOfRecords);
		return count;
	}
	/**
	 * updates the object.
			...
			SessionDataBean sessionDataBean) throws BizLogicException
	{
		try
		{
			...
			{
//				throw new DAOException(ApplicationProperties.getValue("errors.invalid.object.passed"));
				throw new BizLogicException(ErrorKey.getErrorKey("errors.invalid.object.passed"),null,"object is not instance of ShipmentRequest class");
			}
			ShipmentRequest shipmentRequest = (ShipmentRequest) obj;
			//Do not update site of containers included in the shipment request
			Collection<StorageContainer> containerCollection=updateContainerDetails(
			...
			shipmentRequest.getContainerCollection().addAll(containerCollection);
//			Do not update site of containers included in the shipment request
			Collection<Specimen> specimenCollection=updateSpecimenCollection(shipmentRequest.getSpecimenCollection(),dao,sessionDataBean);
			//Set the collection containing the container objects to the shipment object
			shipmentRequest.getSpecimenCollection().clear();
			shipmentRequest.getSpecimenCollection().addAll(specimenCollection);
...
//				logger.debug(ApplicationProperties.getValue("errors.mail.sending.failed"),new BizLogicException(ErrorKey.getErrorKey("errors.mail.sending.failed"),null,"mail sending failed"));
	...
	public String getObjectId(DAO dao, Object domainObject)
	{
		...
		if (shipmentRequest.getActivityStatus() != null
				&& shipmentRequest.getActivityStatus().equals(
						Constants.ACTIVITY_STATUS_REJECTED))
		{
			if (shipmentRequest.getReceiverSite() != null)
			{
				tempString.append(Site.class.getName()).append("_").append(shipmentRequest.getReceiverSite().getId().toString());
		...
		else
		{
			...
			{
				tempString.append(Site.class.getName()).append("_").append(shipmentRequest.getSenderSite().getId().toString());
				return tempString.toString();
			}
		}
		return null;
	 ...
	 * @throws UserNotAuthorizedException if user is found unauthorized.
	 */
	public void closeDraftedRequest(long requestId, SessionDataBean sessionDataBean,int daoType) throws ApplicationException
	{
