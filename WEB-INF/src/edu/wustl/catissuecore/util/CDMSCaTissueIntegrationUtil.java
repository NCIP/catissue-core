...
{
    private static final  Logger LOGGER = Logger.getCommonLogger(CDMSCaTissueIntegrationUtil.class);
    ...
    public static String formReqParameter(final String parameterName, final String value)
    {
        ...
        str.append(CDMSIntegrationConstants.DELIMITER).append(parameterName).append(CDMSIntegrationConstants.EQUALS)
                .append(value);
        return str.toString();
    }
...
* @throws BizLogicException
   */
  public Map getSCGRelatedEncounteredDate_OLD(final Long cpId,final Long pId,final Long cpeId,final Long scgId,Date currDate,Date prevDate) throws BizLogicException
  {
      final StringBuilder scgHql = new StringBuilder();
      Map map = new HashMap();
      final DefaultBizLogic bizLogic = new DefaultBizLogic();
   ...
   * @throws BizLogicException
   */
  public void getSCGRelatedEncounteredDate(CatissueCdmsURLInformationObject informationObject)
			throws BizLogicException
	{
		final StringBuilder scgHql = new StringBuilder();
		final DefaultBizLogic bizLogic = new DefaultBizLogic();
		Long cprId = getCPRId(Long.valueOf(informationObject.getParticipantIdentifier()), Long
				.valueOf(informationObject.getCollectionProtocolIdentifier()));
		scgHql.append("select scg.id from ").append(SpecimenCollectionGroup.class.getName())
				.append(" as scg");
		scgHql.append(" where scg.collectionProtocolEvent.id= ").append(
				informationObject.getCollectionProtocolEventIdentifier());
		scgHql.append(" and scg.collectionProtocolRegistration.id=").append(cprId);
		scgHql.append(" and scg.activityStatus='").append(Constants.ACTIVITY_STATUS_VALUES[1])
				.append("'");
		scgHql.append(" and scg.collectionStatus='").append(Constants.COMPLETE).append("'");
		scgHql.append(" order by scg.id");
		final List scgColl = bizLogic.executeQuery(scgHql.toString());
		...
		for (int i = 0; i < scgColl.size(); i++)
		{
					...
					informationObject.getSpecimenCollectionGroupIdentifier()))
			{
				...
				informationObject.setRecentSpecimenCollectionGroupDate(currDate);
				informationObject.setPreviousSpecimenCollectionGroupDate(prevDate);
				break;
			}
		}
	}
         ...
         Long visitId) throws BizLogicException
 {
     ...
     try
     {
         ...
         Date visitPrevDate = map.get(CDMSIntegrationConstants.PREV_VISIT_DATE);
         Date visitCurrDate = map.get(CDMSIntegrationConstants.RECENT_VISIT_DATE);
 	...
 	public String getRecentSCG(Long collectionProtocolId, Long participantId,
			Long collectionEventId, Date visitPrevDate, Date visitCurrDate) throws BizLogicException
	{
		CDMSCaTissueIntegrationUtil util = new CDMSCaTissueIntegrationUtil();
		String scgId = null;
		try
