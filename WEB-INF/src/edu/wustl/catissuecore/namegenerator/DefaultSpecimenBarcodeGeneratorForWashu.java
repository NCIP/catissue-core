...
public class DefaultSpecimenBarcodeGeneratorForWashu implements BarcodeGenerator
{
	 ...
	 * of Base class.When getInstance of base class called then this init
	 * function will be called.
	 * This method will first check the Datatbase Name and then set function
	 * name that will convert lable from int to String
	 */
	protected void init()
	{
		try
		{
			...
			{
				currentBarcode = getLastAvailableSpecimenBarcode
								(Constants.ORACLE_MAX_BARCODE_COL);
			}
			else if (Constants.MSSQLSERVER_DATABASE.equals(Variables.databaseName))
			{
				currentBarcode = getLastAvailableSpecimenBarcode
								(Constants.MSSQLSERVER_MAX_BARCODE_COL);
			}
			else
			{
	 ...
	 * @param databaseConstant database constant
	 * @return noOfRecords
	 */
